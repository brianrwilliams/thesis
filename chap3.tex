...
On $\CC^d$ exists a natural action by the group $U(d) \ltimes \CC^d$, where $\CC^d$ acts by translations and $U(d)$ acts in the defining way on $\CC^d$. 
For each $\theta \in \Sym^{d+1}(\fg^*)^\fg$ the functional $J_{\CC^d}(\theta)$ is invariant for this group.
In fact, this describes up to quasi-isomorphism all such functionals.

\begin{prop}
The map $J_{\CC^d} :  \Sym^{d+1}(\fg^*)^\fg [-1] \to \cloc^*(\sG_{\CC^d})$ factors through the subcomplex of local cochains that are invariant for the group $U(d) \ltimes \CC^d$ to define a quasi-isomorphism
\ben
J_{\CC^d} : \Sym^{d+1}(\fg^*)^\fg [-1] \xto{\simeq} \cloc^*(\sG({\CC^d}))^{U(d) \ltimes \CC^d}
\een
\end{prop}
...
\section{Local structures of the Kac-Moody factorization algebra}
...
The cocycle is of total cohomological degree $+2$ and so determines a central extension of $A_d \tensor \fg$ that we denote $\fg_{d,\theta}$. 
...
Moreover, one has
\ben
\Theta_V[L] = \sum_{\substack{\Gamma \in \text{\rm Wheels}\\ e \in {\rm Edge}(\Gamma)}} W_{\Gamma,e}(P_{\epsilon<1}, K_\epsilon,
I^{\rm \fg}[\epsilon]),
\een
...
\begin{prop}[\cite{CG} Lemma 12.7.0.1] 
...
\begin{eg}
Consider the $\beta\gamma$ system on a complex manifold $X$ with values in a vector space $W$.  
The fields $\sE_W$ consist of elements $\gamma \in \Omega^{0,*}(X,W)$ together with their conjugates. 
The dg Lie algebra $\sT(X)$ acts on the cochain complex $\Omega^{0,*}(X,W)$ via Lie derivative: if $\xi \in \sT(X)$ of degree $k$ and $\gamma \in \Omega^{0,l}(X)$ then $L_\xi \gamma \in \Omega^{0,k+l}(X, W)$ is defined. 
...
\ben
I^{\sT}(\xi, \beta,\gamma) = \int \<\beta, L_\xi \gamma\>_W ,
\een
where $\<-,-\>$ is, as usual, the pairing between $W$ and its dual.
This functional defines a Maurer-Cartan element in
\ben
I^{\sT} \in {\rm Act}(\sT, \sE_W) [-1] \subset \cred^*(\sT(X)) \tensor \oloc(\sE_W)[-1]
\een
and hence we have a classical action of $\sT_X$ on $\sE_W$. 
\end{eg}
...
The interesting deformations of this factorization algebra come from local cocycles on $\sT_X$ which allow us to define the twisted enveloping algebra.

\begin{dfn}
Let $\alpha \in H^1_{loc}(\sT_X)$.
The {\em Virasoro factorization algebra} on $X$ of central charge $\alpha$ is the twisted factorization enveloping algebra
$\UU_\alpha(\sT_X)$. 
\end{dfn}
...
Implicit in the statement below is the relationship between one-dimensional holomorphic factorization algebras and vertex algebras that we recalled at the beginning of Section \ref{??}.
...
Our aim is to classify the space of central charges of a holomorphically covariant quantum field theory.
...
\ben
\left. \left(\CC[a_1,\ldots, a_{2d-1}, \tau_1,\ldots,\tau_d]\right) \right/ \left(c_1^{j_1}\cdots c_d^{j_d} \right),
\een
