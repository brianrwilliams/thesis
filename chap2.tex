...
\ben
\frac{1}{(-2\pi i)^k k!} {\rm Tr}({\rm At}^\GF(\hT_n)^{\wedge k}): (X_1,\ldots,X_k) \mapsto \frac{1}{(-2\pi i)^k k!} {\rm Tr} \left(\d_{dR}(\Jac(X_1)) \wedge \cdots \wedge \d_{dR}(\Jac(X_k))\right)
\een 
...
Explicitly:
\be\label{k formula}
\alpha_k : (f_1^i \partial_i, \ldots, f_k^i \partial_i) \mapsto \frac{1}{(-2\pi i)^k k!}  {\rm Tr} \left(\Jac(X_1) \wedge \d_{dR} (\Jac(X_2)) \wedge \cdots \wedge \d_{dR}(\Jac(X_k))\right) .
\ee
\end{cor}
...
\ben
\left(\d \omega + \frac{1}{2}[\omega,\omega]\right) + \left(\d + [\omega,-]\right) \alpha + \frac{1}{(d+1)!} \ell_{d+1} (\omega,\ldots,\omega) .
\een
...
\ben
\Def^{\rm cot}_{Y \to B\fg} = \left(\oloc(\Omega^{0,*}_Y \tensor \fg), \dbar + \{I_{Y \to B \fg}, -\}\right)
\een
...
\ben
\left(\oloc(\Omega^{0,*}_Y \tensor \fg), \dbar + \{I_{Y \to B \fg}, -\}\right) = \cloc^*(\Omega^{0,*}_Y \tensor \fg) .
\een
We defined $\cloc^*(\sL)$ of a local Lie algebra in Section \ref{sec: equivariant??}.
...
On $\CC^d$ there is the natural action of the affine group $U(d) \ltimes \CC^d$. 
Here, $U(d)$ acts on $\CC^d$ via the defining representation, and $\CC^d$ acts via translations. 
The group $U(d) \ltimes \CC^d$ act as automorphisms on any tensor bundle on $\CC^d$ and hence acts on sections via the pull back. 
...
The action is by the identity on $\fg$, so that the $L_\infty$ structure maps are compatible with this action of the affine group. 
As with a group action on any elliptic moduli problem, this extends to one on the cotangent theory in a way that preserves the $(-1)$-shifted symplectic pairing. 
Thus, we have the following.

\begin{lem}
The classical theory of holomorphic maps $\CC^d \to B \fg$, $\sE_{\CC^d \to B \fg} = T^*[-1](\Omega^{0,*}(\CC^d) \tensor \fg[1])$ is equivariant for the group of affine symmetries $U(d) \ltimes \CC^d$.
When $\fg = \CC^n[-1]$ the action of $U(d) \ltimes \CC^d$ on $\sE_n$ is compatible with the action of the Harish-Chandra pair $(\Vect, \GL_n)$. 
\end{lem}

The second statement of the lemma is immediate since the actions of $U(d) \ltimes \CC^d$ and $(\Vect, \GL_n)$ commute.
...
\een
Moreover, $J_\omega$ is $U(d) \ltimes \CC^d$-invariant.
\end{lem}
\begin{proof}
...
\ben
J : \Omega^{d+1}_{cl}(B \fg) [d] \xto{\simeq} \left(\Def_{\CC^d \to B \fg}\right)^{\CC^d \ltimes U(d)} .
\een
\end{thm}
...
\ben
J : \hOmega^{d+1}_{n,cl} [d] \xto{\simeq} \left(\Def_n\right)^{\CC^d \ltimes U(d)} .
\een
\end{cor}

This induces a quasi-isomorphism into the $(\Vect, \GL_n)$-equivariant deformation complex
\ben
J^{\rm W} : \clie^*(\Vect , \GL_n ; \hOmega^{d+1}_{n,cl}) \xto{\simeq} \left(\Def^{\rm W}_n\right)^{\CC^d \ltimes U(d)} .
\een
...
\ben
J^X : \Omega^{d+1}_{X,cl}[d] \xto{\simeq}  \left(\Def_{\CC^d \to X} \right)^{\CC^d \ltimes U(d)} .
\een
...
The moduli of cotangent quantizations that are $U(d) \ltimes \CC^d$-invariant is controlled by the extended deformation complex 
\be\label{extended invariant}
\left(\Tilde{\Def}^{W}_n\right)^{U(d) \ltimes \CC^d}
\ee
The $U(d) \ltimes \CC^d$-invariant deformation complex $(\Def_n)^{U(d) \ltimes \CC^d}$ is quasi-isomorphic to $\hOmega^{d+1}_{n,cl}[d]$.
...
\ben
H^{*}(\Vect \oplus \hOmega^{d+1}_{n,cl} [d-1] , \GL_n ; \hOmega^{d+1}_{n,cl}) .
\een
\brian{finish}
\end{proof}
...
Under the characteristic map ${\rm char}_X(I^{\rm W}) \in \Def_{\CC^d \to X}$ determines a solution to the $\Omega^*_X$-linear classical master equation.
...
An immediate corollary of our main result in the previous section, Theorem \ref{thm: extended quantization}, is the following
...
\een
defines a quantization of the holomorphic $\sigma$-model of maps $\CC^d \to X$.
\end{thm}
...
The higher dimensional calculation is similar.

By the local operators of the theory we mean those supported on the disk $D^2 \subset \CC^2$. 
...
\end{prop}
\begin{proof}
We recall the basis for a dense subspace of the observables on a $2$-disk that we described in Section \ref{sec: disk observables}.
