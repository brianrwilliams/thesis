...
It is a general result of Costello \cite{CostelloHolomorphic, WG2} that $\sigma$-models in the BV formalism can be represented as maps from a elliptic ringed space to an $L_\infty$ space.
...
By definition, an $L_\infty$ space is a manifold $X$ with a sheaf of curved $L_\infty$ algebras $\fg$ defined over the de Rham complex $\Omega^*_X$. 
...
\begin{lem}
The Gelfand-Kazhdan descent $\desc_X(\Omega^{0,*}(Y) \tensor \fg_n)$ has the structure of a curved $L_\infty$ algebra over $\Omega^*_X$ and is equivalent to $\Omega^{0,*}_Y \tensor \fg_{X_{\dbar}}$.
\end{lem}
...
\ben
{\rm char} : \clie^*(\Vect , \GL_n ; \oloc(\sE_n)) \to \oloc(\sE_{Y \to X})
\een
...
We have already seen how formal geometry allows us to descend the classical $(\Vect,\GL_n)$-equivariant formal $\beta\gamma$ system $\sE_n$ to the holomorphic $\sigma$-model with arbitrary complex target. 
...
Forgetting analytic difficulties for a moment, we can exhibit why our formal quantization produces one ...
Let us fix an extended Gelfand-Kazhdan structure as in Section \ref{sec: extended geometry}.
This was given by a complex manifold $X$ and a trivialization $\alpha$ of $\ch_{d+1}(T_X^{1,0})$.
