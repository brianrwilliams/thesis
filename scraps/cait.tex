...
\een
%There is an obvious projection map $\Omega^{-*}(T^*V) \to \Omega^{-*}(V)$ that can be thought of as pulling back along the zero section.
%At the level of fields, we this is the restriction of global observables to ones that only depend on the $\gamma$ field. 
...
\ben
\Tilde{\int_{BV}} : \clie^*\left(\Vect, \GL_n; \left( \Omega^{-*}_n \tensor_{\hO_n}\Hat{\rm PV}_n \tensor \Hat{\rm PV}_n^{-*}\right)  [[\hbar]], \hbar L_\pi \right) \to \clie^*(\Vect ; \CC)[[\hbar, \hbar^{-1}] [2n]
\een
...
\ben
\clie^*\left(\Vect, \GL_n; \left( \Omega^{-*}_n \tensor_{\hO_n}\Hat{\rm PV}_n \tensor \Hat{\rm PV}_n^{-*}\right)\right)
\een
...
%\ben
%\xymatrix{
%H^{2n}\left(\Vect, \GL_n ; \hOmega^{-*}_n \tensor_{\hO_n} \Sym(\hT_n \oplus \hT_n [1])\right) \ar[d]_-{\int_{BV}} \ar[r] & H^n\left(\Vect, \GL_n ; \hOmega^{-n}_n \right) \ar[dl]^-{\psi} \ar[dd]^-{{\rm char}_X} \\
%H^{2n}(\Vect, \GL_n; \CC) \ar[d]_{\rm char_X} & \\
%H^{2n}(X , \CC) & \ar[l] H^n(X , \Omega^{n}_X) & .  
%}
%\een
%\end{prop}
