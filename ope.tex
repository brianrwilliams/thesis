...
Then, we study what structure the factorization product endows the operators on $S^{2d-1}$. 

Heuristically speaking, the operators we will consider are supported on spheres.
Since the factorization algebra only takes values on open sets, we need to fix small neighborhoods of the spheres in order to talk about observables.
We describe the precise open neighborhoods of the $(2d-1)$-sphere that we will consider.
...
This will endow the cochain complex $\sA_V^\hbar$ with the structure of an associative (really $A_\infty$) algebra. 
...
This will tell us what the (strict) associative algebra structure is on the cohomology $H^*(\sA_V)$. 
Second, we will use the smoothed description of the observables to nail down the precise $A_\infty$ structure. 
...
Our main result is that the $A_\infty$ algebra structure on $\sA_V$ endowed by the factorization product is equivalent to the universal enveloping algebra $U(\sH_V)$ of the dg Lie algebra $\sH_V$.
...
\begin{align*}
\left|\cO_{\gamma,\alpha}(0;v^*)\right| = |\alpha| - d + 1 \\
\left|\cO_{\beta, z_1^{-1}\cdots z_d^{-1} \alpha'}(0;v)\right| = |\alpha'| .
\end{align*}
In cohomology, the only nontrivial degrees of $\alpha,\alpha'$ that survive are $0,d-1$. 
...
\ben
\Omega^{d,*}_c(U) \tensor V^*[d] \oplus \Omega^{0,*} (U) \tensor V [1] \subset \Obs^{\cl, sm}_V(U) .
\een
...
An elementary observation identifies the smoothed quantum observables with the factorization enveloping algebra of $\sH'_V$:
\ben
\Obs^{\q,sm}_{V} \cong \UU(\sH_V^{sm}) .
\een
Indeed, the right hand side assigns to each open $U$ the cochain complex $\clieu_*(\sH_V'(U)) = \left(\Sym(\sH_V'(U)), \dbar + \d_{CE}\right)$. 
...
%\een
\brian{call $\Tilde{A}_V$ the algebra with commutator that includes the error term in the residue}.
