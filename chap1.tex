...
The functional $W(P_{\epsilon < L},-)$ is defined as a sum over weights of graphs.
This is how Feynman diagrams appear in Costello's formalism.
...
When one studies the {\em observables} of a QFT \brian{finish}
\end{rmk}
...
\ben
S = \int_X \ev_{\fg}(B, \d A) + \frac{1}{3} \ev_{\fg}(B, [A,A]) .
\een
...
& \displaystyle \times \sum_{i_1,\ldots,i_{k-1} =1}^d \epsilon_{i_1\cdots,i_k} \left(\frac{\wbar^1_{i_1}}{t_1} \frac{(\wbar^1)^{n^1}}{t^{|n^1|}}\right) \cdots \left(\frac{\wbar^{k-1}_{i_{k-1}}}{t_{k-1}}\frac{(\wbar^{k-1})^{n^{k-1}}}{t^{|n^{k-1}|}}\right) \left(\sum_{\alpha=1}^{k-1} \frac{\wbar^\alpha_{i_k}}{t_k} \cdot \frac{1}{t^{|n^k|}} \left(\sum_{\alpha=1}^{k-1} \wbar^\alpha\right)^{n^k}\right) \\
...
This expression for the obstruction fits into a generic class of of one-loop anomalies from gauge theory called the Adler-Bardeen-Jackiw (ABJ) anomaly \cite{Adler, BJ}.
...
The fundamental calculation is the infamous "triangle diagram", see Figure \ref{fig: triangle}, where two vertices are labeled by the gauge field and the third by $\alpha$.
