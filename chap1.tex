...
Many ordinary categories of topological vector spaces embed inside the category of differentiable vector spaces. 
...
One can show, Proposition B.3.0.6, that this defines a functor ${\rm dif}_t : {\rm LCTVS} \to DVS$ sending $V$ to the $C^\infty$-module $M \mapsto C^\infty(M, V)$.
If ${\rm BVS} \subset {\rm LCTVS}$ is the subcategory with the same objects but whose morphisms are bounded, this functor restricts to embed ${\rm BVS}$ as a full subcategory ${\rm BVS} \subset {\rm DVS}$. 
...
The full subcategory ${\rm dif}_c : {\rm CVS} \subset {\rm DVS}$ is closed under limit, countable coproducts, and sequential colimits of closed embeddings. 
...
We will not define the tensor product here, but refer the reader the cited reference for a complete exposition.
...
\ben
\Sym^n(V^*) = {\rm Hom}^{multi}_{\rm DVS}(V \times \cdots \times V, C^\infty)_{S_n}
\een
...
\ben
\sO(\sE(X)) \cong \prod_{n} \sD_c(X^n , (E^!)^{\boxtimes n})
\een
...
The class of functionals $S : \sE \to \RR$ defining the classical theory we consider are required to be {\em local}, or given by the integral of a Lagrangian density. 
...
We refer to $\sO_{red}(J(E))$ as the space of {\em Lagrangians} on the space of sections $\sE(X)$. 
Indeed, every element $F \in \sO_{red}(J(E))$ can be expanded as $F = \sum_n F_n$ where each $F_n$ is an element 
\ben
F_n \in {\rm Hom}_{C^\infty_X} (J(E)^{\tensor n}, C^\infty_X)_{S_n} \cong {\rm PolyDiff}(\sE^{\tensor n}, C^\infty(X))
\een
...
\ee
Often times when we describe a local functional we will write down its value on test compactly supported sections, then check that it is given by integrating a Lagrangian density.

\subsubsection{The definition of a classical field theory}

Before giving the definition, we need to recall what the proper notion of a shifted symplectic structure is in the infinite dimensional setting that we work in.

\begin{dfn}
Let $E$ be a graded vector bundle on $X$.
A $(-1)$-{\em shifted symplectic structure} is an isomorphism of graded vector spaces
\ben
E \cong_{\omega} E^![-1] = \left({\rm Dens}_X \tensor E^\vee\right)[-1]
\een
...
\begin{dfn}
A {\em classical field theory} on a smooth manifold $X$ is a $\ZZ$-graded vector bundle $E$ equipped with a $(-1)$-shifted symplectic structure together with a local functional $S \in \oloc(\sE)$ such that:
\begin{enumerate}
...
Thus, a free theory is a simply an elliptic complex equipped with a shifted symplectic pairing. 

Although the space $\sO(\sE)$ does not have a well-defined shifted Possoin bracket induced from the symplectic pairing, the operator $\{S,-\} : \sO(\sE) \to \sO(\sE)[1]$ {\em is} well-defined. 
By assumption, it is also square zero. 
The complex of global observables of the theory is defined by
\ben
...
\begin{dfn}
A {\em holomorphically translation invariant} local functional is a translation invariant local functional $I \in \oloc(\sE_V)^{\CC^d}$ such that $\eta_i I = 0$ for all $1 \leq i \leq d$. 
\end{dfn}
...
\ben
\CC \left[\d \zbar_i, \eta_i, \frac{\partial}{\partial z_i}, \frac{\partial}{\partial \zbar_i} \right] .
\een
...
Let $(V,Q^{hol},(-,-)_V, I)$ be the data of an interacting holomorphically translation invariant theory on $\CC^d$.
...
\een
There is an induced action of translations $\CC^{2d}$ on this cochain complex. 
The condition of translation invariance was not the only one required to have a holomorphically translation invariant local functional.
We also require that $\eta_i I = 0$.
The abelian Lie alegebra spanned by the odd elements $\{\eta_i\}$ is $d$-dimensional and we denote it by $\CC^d[1]$.
We want to consider deformations that are invariant for the action by the total Lie algebra $\CC^{2d} \oplus \CC^d[1]$.
We let $(\Def_{\sE_V})^{\CC^d \oplus \CC^d[1]}$ be the space of such invariant deformations.

Recall, in the definition of the local functionals on a graded vector bundle one considers (reduced) functionals on the jet bundle $JV$.
In the holomorphic case there is also a holomorphic jet bundle $J^{hol}V$.
...
\ben
\left(\Def_{\sE_V}\right)^{\CC^{2d} \oplus \CC^d[1]} \simeq \CC \cdot \d^d z \tensor^{\LL}_{\CC[\partial_{z_1}, \ldots, \partial_{z_d}]} \sO_{red}(V_0[[z_1,\ldots,z_d]])[d].
\een
...
\ben
\left(\Def_{\sE_V}\right)^{\CC^{2d} \oplus \CC^d[1]}\simeq \CC \cdot \d^d z \tensor^{\LL}_{\CC[\partial_{z_1}, \ldots, \partial_{z_d}]} \cred^*\left(V_0[[z]][-1])\right) [d].
\een
...
\ben
\CC[\partial_{z_i}] \hookrightarrow \CC[\partial_{z_i}, \partial_{\zbar_i}, \Bar{\eta}_i].
\een
...
Let $\sE$ be the underlying space of fields.
