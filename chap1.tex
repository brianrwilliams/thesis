...
We require that $D = [Q,Q^{GF}]$ is a generalized Laplacian in the sense of \cite{??}, in addition to other conditions that can be found in Definition 5.4.0.5 in \cite{CG2}. 
...
Indeed, for any $\epsilon, L > 0$ introduce the {\em propagator} 
\ben
P_{\epsilon < L} = \int_{t = \epsilon}^L (Q^{GF} \tensor 1) K_t \d t .
\een 
...
\een
This bracket is defined on all of $\sO(\sE)$. 
