...
\ben
\xymatrix{
{\rm Hom}({\rm Jet}^{hol}(V)^{\tensor n} , K_X \tensor K_X) \ar[r]^-{(-,-)^{-1}_V} & {\rm Hom}({\rm Jet}^{hol}(V)^{\tensor (k +k' -2)} , K_X^* \tensor K_X \tensor K_X) \ar[d] \\ & {\rm Hom}({\rm Jet}^{hol}(V)^{\tensor (k +k' -2)} , K_X).
}
\een
...
& \displaystyle \times\left( \sum_{i_1,\ldots, i_k} \epsilon_{i_1\cdots,i_d} D_{1, i_1} \cdots D_{k-1,i_{k-1}} \sum_{\alpha=1}^{k-1} D_{\alpha, i_k} \Phi(w^1,\ldots,w^k) \right) \\ & \displaystyle \times  \exp\left(- \sum_{\alpha=1}^{k-1} \frac{|w^{\alpha}|^2}{t_\alpha} - \frac{1}{t_k} \left|\sum_{\alpha=1}^{k-1} w^\alpha \right|^2\right) .
\end{array}
\een
...
\begin{align} \label{weight bound1}
|W_{\epsilon < L}^{k, (n)}(\Phi)| \leq & \;\; C \int_{w^k \in \CC^d} \d^{2d} w^k \int_{(w^1,\ldots,w^{k-1}} \prod_{\alpha=1}^{k-1} \d^{2d} w^\alpha \Psi(w^1,\ldots,w^{k-1},w^k)\\ & \times \int_{(t_1,\ldots,t_k) \in [\epsilon,L]^k} \d t_1 \ldots \d t_k \frac{1}{(4\pi)^{dk}} \frac{1}{t^d_1\cdots t^d_k} \times E(w,t)
\end{align}
...
\een
that sends a smooth compactly supported function $\Phi \in C_c^\infty((\CC^d)^k) = C_c^\infty(\CC^{dk})$ to
\begin{align}\label{anomalyweight1}
\Tilde{W}_{\epsilon < L}^{k, (n)} (\Phi) = & \int_{(z^1,\ldots, z^k) \in (\CC^d)^k} \prod_{\alpha=1}^k \d^d z^\alpha \Phi(z^1,\ldots,z^\alpha)  \left(\frac{\partial}{\partial z^k}\right)^{\vec{n}^k} K_{\epsilon}(z^1,z^k) \\ & \times \prod_{\alpha = 1}^{k-1} \left(\frac{\partial}{\partial z^\alpha}\right)^{\vec{n}^\alpha} P_{\epsilon < L}^{an}(z^\alpha, z^{\alpha+1}) .
\end{align}
...
\ben
\begin{array}{lll}
\Tilde{W}_{\epsilon < L}^{k, (n)}(\Phi) & = \displaystyle \int_{w^k \in \CC^d} \d^{2d} w^k \int_{(w_1,\ldots,w_{k-1}) \in (\CC^d)^{k-1}} \left(\prod_{\alpha=1}^{k-1} \d^{2d} w^\alpha\right) \Phi(w^1,\ldots,w^k) \\ & \times \displaystyle \int_{(t_1,\ldots,t_k) \in [\epsilon,L]^{k-1}} \frac{1}{(4 \pi \epsilon)^d} \prod_{\alpha=1}^{k-1} \frac{\d t_\alpha}{(4 \pi t_\alpha)^d} \\
& \displaystyle \times \sum_{i_1,\ldots,i_{k-1} =1}^d \epsilon_{i_1,\ldots,i_d} \left(\frac{\wbar^1_{i_1}}{t_1} \frac{(\wbar^1)^{n^1}}{4t^{|n^1|}}\right) \cdots \left(\frac{\wbar^{k-1}_{i_{k-1}}}{4t_{k-1}}\frac{(\wbar^{k-1})^{n^{k-1}}}{4t^{|n^{k-1}|}}\right)  \\
& \displaystyle \times \left(\frac{1}{4t^{|n^k|}} \left(\sum_{\alpha=1}^{k-1} \wbar^\alpha\right)^{n^k}\right) \exp\left(- \sum_{\alpha=1}^{k-1} \frac{|w^{\alpha}|^2}{4t_\alpha} - \frac{1}{4\epsilon} \left|\sum_{\alpha=1}^{k-1} w^\alpha \right|^2\right) .
\end{array}
\een
...
\begin{align*}
D_{\alpha, i_\alpha}(t) := & \left(\frac{\partial}{\partial w^\alpha_{i_\alpha}} - \sum_{\beta = 1}^{k-1} \frac{t_\beta}{t_1+\cdots + t_{k-1} + \epsilon} \frac{\partial}{\partial w_{i_\alpha}^{\beta}}\right) \times \\ & 
...
\ben
\begin{array}{lll}
\Tilde{W}_{\epsilon < L}^{k, (n)}(\Phi) & = \displaystyle \pm \int_{w^k \in \CC^d} \d^{2d} w^k \int_{(w_1,\ldots,w_{k-1}) \in (\CC^d)^{k-1}}\left(\prod_{\alpha=1}^{k-1} \d^{2d} w^\alpha\right) \\ & \displaystyle \times \int_{(t_1,\ldots,t_{k-1}) \in [\epsilon,L]^{k-1}} \frac{1}{(4 \pi \epsilon)^d} \prod_{\alpha=1}^{k-1} \frac{\d t_\alpha}{(4 \pi t_\alpha)^d}  \\ 
& \displaystyle \times\left( \sum_{i_1,\ldots, i_{k-1}} \epsilon_{i_1\cdots,i_d} D_{1, i_1}(t) \cdots D_{k-1,i_{k-1}} (t) D_k(t) \Phi(w^1,\ldots,w^k) \right) \\ & \displaystyle \times \exp\left(- \sum_{\alpha=1}^{k-1} \frac{|w^{\alpha}|^2}{4t_\alpha} - \frac{1}{4\epsilon} \left|\sum_{\alpha=1}^{k-1} w^\alpha \right|^2\right) .
\end{array}
\een 
