...
such that 
\begin{itemize}
\item[(1)] for all $X \in \Vect$, $f \in \hO_n$ and $v \in \sV$ we
...
$(\Vect, \GL_n)$-modules $f : \sV \to \sV'$. We denote this category
...
The internal hom $\Hom_{\hO}(\sV,\sW)$ also provides a vector bundle on the formal disk, 
...
\[
\RR\Hom_{D}(\hO, \sV) \simeq \clie^*(\Vect ; \sV),
\]
or more accurately a zig-zag of quasi-isomorphisms. Here
$\clie^*(\Vect ; \sV)$ is the continuous cohomology of $\Vect$ with
coefficients in $\sV$. This is known as the {\em Gelfand-Fuks}
cohomology of $\sV$ and is what we use for the remainder of the
paper. 
...
\[
\clie^*(\Vect,\GL_n; \sV) \simeq \RR \Hom_D(\hO,\sV)^{\GL_n-{\rm eq}},
\]
...
Given a finite-dimensional $\GL_n$-representation $V$, we construct a $\hO_n$-module $\sV \in \VB_n$ as follows. 
...
\subset \Vect$ to get a $\Vect$-module $\sV = \Hom_{\fm_n^1 \cdot \Vect}(\Vect,V)$. 
There is an induced action of $\GL_n$ on $\sV$. Indeed, as a $\GL_n$-representation one has $\sV \cong \hO_n \tensor_{\CC} V$.
Moreover, this action is compatible with the $\Vect$-module structure, so that $\sV$ is actually a $(\Vect, \GL_n)$-module. 
...
Note that every formal vector bundle $\sV \in \WGLCAT$ is naturally filtered via a filtration inherited from $\hO_n$. 
Explicitly, we see that $\sV$ is the limit of the sequence of finite-dimensional vector spaces
\[
...
In this way, $\sV$ produces a natural sequence of vector bundles on $X$ and thus a pro-vector bundle on $X$.
...
--- and a formal vector bundle $\sV$ 
to the pro-vector bundle $\Fr_X \times^{\GL_n} \sV$ with flat connection induced by the Grothendieck connection.
\end{dfn}
...
\begin{proof}
For every $\sV,\sW$ in $\WGLCAT$, we have natural maps
\[
(\Omega^*(\Fr_X) \otimes \sV)_{bas} \otimes (\Omega^*(\Fr_X) \otimes \sW)_{bas} \to (\Omega^*(\Fr_X) \otimes (\sV \otimes \sW))_{bas} \to (\Omega^*(\Fr_X) \otimes (\sV \otimes_{\hO_n} \sW))_{bas}
\]
...
\begin{prop}
For $\sV \in \VB_n$ corresponding to the $\GL_n$-representation $V$,
there is a natural isomorphism of flat pro-vector bundles
\[
\desc((X, \sigma),\sV) \cong {\rm Jet}^{hol}(\Fr_X
\times^{\GL_n} V)
\]
...
We will use the notation $\desc(\sV)$ to denote the functor $\desc(-,\sV): \GK_n^\op \to \Pro(\vb)_{flat}$, 
since we want to focus on the sheaf on $\GK_n$ (or $\Hol_n$) defined
by each formal vector bundle~$\sV$. Taking flat sections we get an
$\cO$-module $\sdesc(\sV)$ which is locally free of finite
...
In particular, once we produce the $\hc$-Atiyah sequence for a formal tensor field $\sV$, 
we will have a very local model for the Atiyah class living in $\clie^*(\Vect,\GL_n; \hOmega^1_n \otimes_{\hO_n} \End_{\hO_n}(\sV))$.

%\owen{For $\sV$ a formal tensor field, there is a natural Lie algebra inclusion $\Vect \to \End_\CC(\sV)$ in addition to the Lie algebra inclusion $\End_{\hO_n}(\sV)) \to \End_\CC(\sV)$.
%By direct computation (e.g., picking an $\hO_n$ frame for $\sV$), one can show that the commutator of a vector field with an $\hO_n$-linear endomorphism is again an $\hO_n$-linear endomorphism.
%Hence these subalgebras together span a Lie subalgebra, which we denote $\cA(\sV)$ for the {\em Atiyah algebra} of $\sV$.
%In fact, these Lie algebras sit in an exact sequence
%\[
%\End_{\hO_n}(\sV) \to \cA(\sV) \xto{\sigma} \End_{\hO_n}(\sV) \otimes_{\hO_n}\hT_n.
%\]
%This sequence splits as $\hO_n$-modules (e.g., by picking an $\hO_n$ frame for $\sV$ and coordinates on the formal $n$-disk), 
%but it is also a sequence of formal tensor fields and typically does not split in $\vb(\hD^n)_{\hc}$.}

%\owen{\begin{dfn}
%The {\em $\hc$-Atiyah sequence} of $\sV$ is the short exact sequence
%\[
%\End_{\hO}(\sV) \to \cA(\sV) \to \hT_n
%\]
%in $\vb(\hD^n)_{\hc}$.
%\end{dfn}
%}
...
Let $\sV$ be a formal vector bundle. 
We will now construct the ``formal'' Atiyah sequence associated to $\sV$.  
First, we need to define the $(\Vect, \GL_n)$-module of {\em first order jets} of $\sV$. 
...
Given an $\cO_X$-module $\sV$, 
``push-and-pull'' along these projections,
\ben
J^k_X(\sV) = (\pi_1^{(k)})_* (\pi_2^{(k)})^* \sV,
\een
defines the $\cO_X$-module of $k$th order jets of~$\sV$.
...
\begin{dfn} 
Let $\sV$ be a formal vector bundle on $\hD^n$.
Consider the $\hO_{2n} / \Hat{I}_n^{k+1}$-module
$\sV \tensor_{\hO_n} \left(\hO_{2n} / \Hat{I}_n^{k+1}\right)$,
where the tensor product uses the $\hO_n$-module structure on the
quotient $\hO_{2n} / \Hat{I}_n^{k+1}$ coming from the map $\pi_2^{(k)*}$. 
We define the {\em $k$th order formal jets of $\sV$}, denoted $J^k(\sV)$, 
as the restriction of this $\hO_{2n}/\Hat{I}_n^{k+1}$-module 
to a $\hO_n$-module using the map $\pi_1^{(k)*} : \hO_n \to \hO_{2n} / \Hat{I}_n^{k+1}$. 
\end{dfn}

\begin{lem} For any $\sV \in \VB_n$ the $k$th order formal jets
  $J^k(\sV)$ is an element of $\VB_n$. 
\end{lem}
\begin{proof}
For $\sV$ in $\VB_n$ there is an induced action of $(\Vect, \GL_n)$ on
the tensor product $\sV \tensor_{\hO_n} \hO_{2n} /
...
As a $\CC$-linear vector space we have $J^1(\sV) = \sV \oplus (\sV \tensor_{\hO_n} \hOmega^1_n)$. 
For $f \in \hO_n$ and $(v, \beta) \in \sV \oplus (\sV \tensor \hOmega^1_n)$, 
...
For any $\sV \in \VB_{\hc}$, the $\hO_n$-module $J^1(\sV)$ has a compatible action of the pair $(\Vect, \GL_n)$ and hence determines an object in $\VB_{\hc}$. 
Moreover, it sits in a short exact sequence of formal vector bundles 
\be\label{formalatiyah1}
\sV \tensor \hOmega^1_n \to J^1 (\sV) \to \sV .
\ee
Finally, the Gelfand-Kazhdan descent of this short exact sequence is isomorphic to the Atiyah sequence
\ben
\sdesc_{\GK}(\sV) \tensor \Omega^1_{hol} \to J^1 \sdesc_{\GK} (\sV) \to \sdesc_{\GK}(\sV) .
\een
In particular, $J^1 \desc_{\GK}(\sV) = \desc_{\GK}(J^1 \sV)$.
\end{prop}

We henceforth call the sequence (\ref{formalatiyah1}) {\em the formal Atiyah sequence} for $\sV$. 

\begin{rmk} 
Note that $J^1(\sV)$ is an element of the category $\VB_n$ but it is {\em not} a formal tensor field. 
...
If we write $\sV = \hO_n \tensor_\CC \sV$, then one defines 
\ben
j^1 : \sV \to J^1 \sV \;\; , \;\; f \tensor_\CC v \mapsto (f \tensor_\CC v, (1 \tensor_\CC v) \tensor_{\cO} \d f) .
\een
It is a map of $\hO_n$-modules, and it splits the obvious projection $J^1(\sV) \to \sV$. 
...
$$\Ext^1(\sV \tensor_{\hO_n} \hOmega_{n}^1, \sV) \cong \clie^1(\Vect,\GL_n; \hOmega^1_n \otimes_{\hO_n} \End_{\hO_n}(\sV))$$ 
and from the observation that an exact sequence in $\vb(\hD^n)$ maps to an exact sequence in $\vb(\GK_n)$.

\begin{cor}
There is a cocycle $\At^\GF(\sV) \in \clie^1(\Vect,\GL_n; \hOmega^1_n \otimes_{\hO_n} \End_{\hO_n}(\sV))$ representing the Atiyah class $\At(\desc(\sV))$. 
\end{cor}

We call this cocycle the Gelfand-Fuks-Atiyah class of $\sV$ since it
descends to the ordinary Atiyah class for $\desc(\sV)$ as a sheaf of
$\cO$-modules. 
...
%\ben
%{\rm H}^1(\Vect, \GL_n ; \hOmega^1_n \tensor \End_{\hO_n}(\sV)) \cong \Ext^1(\sV \tensor \hOmega^1_n, \sV) \to \Ext_{\GK_n}(\desc(\sV) \tensor \Omega^1, \desc(\sV)) .
%\een
...
%\ben
%\Ext^1_{\cO_X}(\sdesc(\sigma, \sV_1), \sdesc(\sigma, \sV_2)) \simeq \bdesc()
%\een
%\end{lemma}

%For a fixed GK-structure $\sigma$ over a complex manifold $X$ the image of $\At^\GF(\sV)$ under t%he characteristic map
%\ben
%\ch_\sigma : \clie^*(\Vect , \GL_n ; \hOmega^1 \tensor_{\hO_n} \End_{\hO_n}(\sV)) \to \bdesc(\sigma, \\sV) = \dR\left(X ; \desc(\sigma, \sV)\right) \simeq \ch^*(X ; \sdesc(\sigma, \sV))
%\een
%agrees with the ordinary Atiyah class for $\desc(\sV)$ in cohomology.

%\owen{\begin{proof}[Proof of lemma]
%The key is to use the Lie algebra structure on the formal Atiyah sequence for $\sV$. 
%The action of formal vector fields on $\sV$ descends to the action of vector fields on the tensor bundle $\desc(\sV)$, which is canonical and determined by the Lie derivative.
%Likewise, the action of $\cO$-linear endomorphisms $\End_{\hO_n}(\sV)$ descends to the action of $\cO$-linear endomorphisms $\End_{\cO}(\desc(\sV))$, which is also canonical.
...
%One strategy would be to reinterpret $\cA(\sV)$ as "invariant vector fields on the total space of $\sV$ over $\hD^n$," although that may seem silly.
%\end{proof}}
...
%%\owen{I'd like to say that $$\loc(\Diff^{\leq 1}_\sV) = \Diff^{\leq 1}_{\loc(\sV)}$$ but I'm not sure how to write something rigorous. The crux seems to be showing that this localized Lie algebra, whose avatar on the formal disk acts naturally on $\sV$ as a ``Lie algebroid,'' acts on the localization of $\sV$ in the appropriate way.}
%
%\owen{Vague thoughts: 
...
\begin{dfn}
The {\em Gelfand-Fuks-Chern character} is the formal sum $\ch^\GF(\sV) = \sum_{k \geq 0} \ch^\GF_k(\sV)$, 
where the $k$th component
\ben
{\rm ch}_k^\GF(\sV) := \frac{1}{(-2 \pi i)^k k!} {\rm Tr}({\At}^\GF(\sV)^k)
\een
lives in $\clie^k(\Vect,\GL_n; \hOmega^k_n)$.
\end{dfn}

It is a direct calculation to see that $\ch^{\GF}_k(\sV)$ is closed for
...
\ben
\At^{\rm GF}(\sV) \in \clie^1(\Vect, \GL_n ; \hOmega^1_n
\tensor_{\hO_n} \End_{\hO}(\sV)) .
\een 
of a formal vector bundle $\sV$. 

By definition, any formal vector bundle has the form $\sV = \hO_n \tensor V$, 
with $V$ a finite-dimensional vector space.
We view $V$ as the ``constant sections'' in $\sV$ by the inclusion $i: v \mapsto 1 \otimes v$.
This map then determines a connection on $\sV$:
we define a $\CC$-linear map $\nabla: \sV \to \hOmega^1_n \otimes_{\hO_n} \sV$
...
\hOmega^1_n \tensor_{\hO} \End_{\hO}(\sV))$, as follows. Let 
\ben
\rho_{\sV} : \Vect \tensor \sV \to \sV
\een
denote the action of formal vector fields and consider the composition
\ben
\Vect \tensor V \xto{\id \tensor i} \Vect \tensor \sV \xto{\rho_{\sV}} \sV \xto{\nabla} \hOmega^1_n \tensor_{\hO} \sV .
\een 
Since $V$ spans $\sV$ over $\hO_n$, this composite map determines a $\CC$-linear map
\[
\alpha_{\sV,\nabla}: \Vect \to \hOmega^1_n \tensor_{\hO} \End_{\hO}(\sV)
\]
by
\[
\alpha_{\sV,\nabla}(X)(fv) = f \nabla( \rho_\sV(X)(i(v))),
...
Let $\sV$ be a formal vector bundle. 
Then $\alpha_{\sV,\nabla}$ is a representative for the Gelfand-Fuks-Atiyah class~$\At^{\rm GF}(\sV)$. 
\end{prop}
...
For a formal vector bundle $\sV = \hO_n \otimes V$, something similar happens after choosing coordinates.
We have $J^1(\sV) \cong \sV \oplus \hOmega^1_n \otimes_{\hO_n} \sV$ and
the 1-jet of an element of $\sV$ can be written as
\ben
\begin{array}{cccc}
j^1 : & \sV & \to & J^1(\sV)   \\
...
determined by $\sV = \hO_n \otimes V$, the defining decomposition.
...
\ben
1 \tensor \nabla : \clie^\#(\Vect ; \sV) \to \clie^\#(\Vect ;\hOmega^1_n
\tensor_{\hO} \sV) .
\een
Let $\d_{\sV}$ denote the differential on $\clie^*(\Vect; \sV)$ and
$\d_{\Omega^1 \tensor \sV}$ denote the differential on \\ $\clie^*(\Vect
; \hOmega^1_n \tensor_{\hOmega} \sV)$. The failure for $1 \tensor \nabla$ is precisely the difference
\be\label{difference}
(1 \tensor \nabla) \circ \d_{\sV} - \d_{\Omega^1 \tensor \sV} \circ (1 \tensor
\nabla).
\ee
This difference is $\clie^\#(\Vect)$ linear and can hence be
thought of as a cocycle of degree one in $\clie^*(\Vect ; \hOmega^1
\tensor_{\hO} \End_{\hO} (\sV))$. This is the representative for the Atiyah
class. 

We proceed to compute this difference. The differential $\d_{\sV}$ splits as $\d_{\Vect}
\tensor 1_\sV
+ \d'$ where $\d_{\Vect}$ is the differential on the complex
$\clie^*(\Vect)$ and $\d'$ encodes the action of $\Vect$ on
$\sV$. Likewise, the differential $\d_{\Omega^1 \tensor \sV}$ splits
as $\d_{\Vect} \tensor 1_{\Omega^1 \tensor \sV} + \d_{\Omega^1}
...
differential that encodes the action of $\Vect$ on $\sV$, so if we
evaluate on an element of the form $1 \in v \in \clie^0(\Vect ; V)
\subset \clie^0(\Vect ; \sV)$ the only term that survives is the GF 1-cocycle
\ben
X \mapsto \nabla \d'(1 \tensor v)(X) = \nabla (\rho_\sV(X) (v)) .
\een
as desired. 
...
We will be most interested in the cocycles $\ch_k(\sV)$ for $k \geq 2$. 
When $k=2$ we obtain a $2$-cocycle with values in $\hOmega^2_{n,cl}$, $\ch_2(\sV) \in \clie(\Vect , \GL_n ; \hOmega^2_{n,cl})$. 
This $2$-cocycle $\ch_2^{\GF}(\sV)$ determines an abelian extension Lie algebras of $\Vect$ by $\hOmega^2_{n,cl}$
\ben
0 \to \hOmega^2_{n,cl} \to \Tilde{\rm W}_{n, \sV} \to \Vect \to 0 .
\een
When $\sV = \hT_n$, denote this extension by $\Tilde{\rm W}_{n, \sV} = \Tilde{\rm W}_{n,1}$. (The notation will become clearer momentarily)
...
For $k > 2$, it will be useful to think of $\ch_k(\sV)$ as defining a similar type of extension.
...
Thus, for any formal vector bundle $\sV$, $\ch_k(\sV)$ determines an abelian $L_\infty$ extension of $\Vect$ by the abelian Lie algebra $\hOmega^{k}_{n,cl}$. 
The case $\sV= \hT_n$ will be especially relevant for us. 
...
Suppose now we fix a trivialization $\alpha$ of the component of the Chern character $\ch_{d+1}(T_X^{1,0})$.
We will resolve $\Omega^{d+1}_{cl}$ by holomorphic vector bundles via the complex
\ben
\Omega^{\geq d+1,hol}_X = \Omega^{d +1,hol}_X \xto{\partial} \Omega^{d+2,hol}_X[-1] \to \cdots .
\een
...
0 \to \hOmega^{d+1}_{n,cl}[d-1] \to \TVectd \xto{\pi_{n,d}} \Vect \to 0 .
\een
The cocycle $\ch_{d+1}(\hT_n)$ also defines an extension by the resolution $\hOmega^{d+1}_{n,cl}[d-1]$
\ben
0 \to \hOmega^{\geq d+1}_{n,cl}[d-1] \to \TVectd ' \xto{\pi'_{n,d}} \Vect \to 0 .
\een
By construction, $\TVectd '$ is quasi-isomorphic to $\TVectd$. 
...
So, up to rescaling $\alpha$, we see that the Maurer-Cartan equation is satisfied. 
...
\xymatrix{
R[1] \ar[r] & \sO (B \fg) \ar[r]^-{\partial} & \Omega^1(B \fg)[-1] \ar[r]^-{\partial}& \cdots \ar[r]^-{\partial} & \Omega^{d}(B \fg)[-d] .
}
\ee
...
\begin{align}\label{U(d) invariants}
...
\begin{align*}
\Theta_{\Gamma,e}(X_\alpha) (\gamma) = & \lim_{\epsilon \to 0} \int_{(z^1,\ldots, z^{d+1}) \in (\CC^d)^{d+1}} \left(\prod_{\alpha=0}^d \d^d z^\alpha\right) \\ & \times \Tr\left(\Jac(X_0) (\gamma)(z_0) \cdots \Jac(X_{d+1})(\gamma)(z_{d+1})\right)
...
& &\displaystyle \sum_{i_1,\ldots,i_d} \epsilon_{i_1,\ldots,i_d} \left(\prod_{\alpha=1}^d \frac{\wbar_{i_{\alpha}}^\alpha}{4 t _\alpha} \right) \prod_{\alpha,i=1}^d \d \wbar_i^\alpha \\ & \times & \displaystyle \exp\left(-\sum_{\alpha=1}^{d} \frac{|w^\alpha|^2}{4 t_\alpha} -\frac{1}{4 \epsilon} \left|\sum_{\alpha=1}^d w^\alpha \right|^2\right) .
\end{array}
\een
...
\begin{align}\label{wick1}
& \int_{w^0\in \CC^d}\d^{2d}w^0  \frac{1}{(4 \pi \epsilon)^d} \int_{(t_1,\ldots,t_k) \in [\epsilon,L]^d} \prod_{\alpha=1}^{d} \frac{\d t_\alpha}{t_\alpha^d} \\ & \times \sum_{i_1,\ldots,i_d} \epsilon_{i_1,\ldots,i_d}  \left. \left(\prod_{\alpha=1}^d \frac{\partial}{\partial w^\alpha_{i_\alpha}} \Psi \right)\right|_{w^1=\cdots=w^d=0}
...
\begin{align*} 
 \int_{w^0\in \CC^d}\d^{2d}w^0  & \left. \sum_{i_1,\ldots,i_d} \epsilon_{i_1,\ldots,i_d}  \left(\prod_{\alpha=1}^d \frac{\partial}{\partial w^\alpha_{i_\alpha}} \Psi \right)\right|_{w^1=\cdots=w^d=0} \\ & \times \frac{1}{(4\pi)^d} \int_{(t_1,\ldots,t_k) \in [\epsilon,L]^d} \frac{\epsilon}{(\epsilon+t_1+\cdots + t_d)^{d+1}} \d t_1 \cdots \d t_d .
 \end{align*}
...
\ben
\begin{tikzcd}    
& \Obs^{\cl}_V(U) = \left(\Sym(\Omega^{d,*}_c(U,V^*)[d] \oplus \Omega^{0,*}_c(U,V)[1]), \dbar\right) \arrow[hook]{d}{\simeq} \\ & \left(\Sym(\Bar{\Omega}^{d,*}_c(U,V^*)[d] \oplus \Bar{\Omega}^{0,*}_c(U,V)[1]), \dbar \right) = \overline{\Obs}_V^{\cl}(U)
\end{tikzcd}
\een
...
We will denote $\sV_V = \Obs_V^\q(r)$, which is well-defined up to quasi-isomorphism by the preceding proposition. 
...
\ben
i_{S^{2d-1}} : A_d \tensor \left(V^*[d-1] \oplus V\right) \to \left(\Omega^{0,*}(N_{r,\epsilon}(w)) \tensor V \oplus \Omega^{d,*}(N_{r,\epsilon}(w)) \tensor V^*[d-1] \right)^\vee
\een
